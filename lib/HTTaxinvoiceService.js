...
BaseService.addMethod(HTTaxinvoiceService.prototype, "requestJob", function (CorpNum, Type, DType, SDate, EDate, UserID, success, error) {
    ...
    var targetURI = '/HomeTax/Taxinvoice/' + Type;
...
BaseService.addMethod(HTTaxinvoiceService.prototype, "search", function (CorpNum, JobID, Type, TaxType, PurposeType, TaxRegIDType, TaxRegIDYN, TaxRegID, Page, PerPage, Order, UserID, SearchString, success, error) {
    if (!JobID || 0 === JobID.length) {
        this._throwError(-99999999, "작업아이디(JobID)가 입력되지 않았습니다.", error);
        return;
    }
    ...
    var targetURI = '/HomeTax/Taxinvoice/' + JobID;
    if (!!Type)
...
BaseService.addMethod(HTTaxinvoiceService.prototype, "summary", function (CorpNum, JobID, Type, TaxType, PurposeType, TaxRegIDType, TaxRegIDYN, TaxRegID, UserID, SearchString, success, error) {
    if (!JobID || 0 === JobID.length) {
        this._throwError(-99999999, "작업아이디(JobID)가 입력되지 않았습니다.", error);
        return;
    }
    ...
    var targetURI = '/HomeTax/Taxinvoice/' + JobID + '/Summary?';
    if (!!Type)
        targetURI += '&Type=' + Type.toString();
    if (!!TaxType)
        targetURI += '&TaxType=' + TaxType.toString();
...
BaseService.addMethod(HTTaxinvoiceService.prototype, "getTaxinvoice", function (CorpNum, NTSConfirmNum, UserID, success, error) {
    if (!NTSConfirmNum || 0 === NTSConfirmNum.length) {
        this._throwError(-99999999, "국세청승인번호(NTSConfirmNum)가 입력되지 않았습니다.", error);
        return;
    }
