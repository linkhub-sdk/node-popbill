...
StatementService.prototype.getUnitCost = function (CorpNum, ItemCode, success, error) {
    ...
    if (!this._checkItemCode(ItemCode)){
        this._throwError('전자명세서 문서유형이 올바르지 않습니다.', error)
        return;
    }
...
StatementService.prototype.attachFileBinary = function (CorpNum, ItemCode, MgtKey, BinaryFiles, UserID, success, error) {
    if (this._isNullOrEmpty(CorpNum)){
        this._throwError('팝빌회원 사업자번호가 입력되지 않았습니다.', error)
        return
    }
...
StatementService.prototype.attachFile = function (CorpNum, ItemCode, MgtKey, DisplayName, FilePath, UserID, success, error) {
    if (this._isNullOrEmpty(CorpNum)){
        this._throwError('팝빌회원 사업자번호가 입력되지 않았습니다.', error)
        return
    }
...
StatementService.prototype.search = function (CorpNum, DType, SDate, EDate, State, ItemCode, QString, Order, Page, PerPage, UserID, success, error) {
    if (this._isNullOrEmpty(CorpNum)){
        this._throwError('팝빌회원 사업자번호가 입력되지 않았습니다.', error)
        return
    }
    ...
    if(!this._isNullOrEmpty(ItemCode)) targetURI += '&ItemCode=' + ItemCode.toString()
...
StatementService.prototype._checkItemCode = function(ItemCode){
    return /^12[1-6]$/.test(ItemCode)
}
