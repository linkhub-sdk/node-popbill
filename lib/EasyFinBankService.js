...
BaseService.addMethod(EasyFinBankService.prototype, 'registBankAccount', function (CorpNum, BankAccountInfo, UserID, success, error) {
...
BaseService.addMethod(EasyFinBankService.prototype, 'deleteBankAccount', function (CorpNum, BankCode, AccountNumber, UserID, success, error) {
    if (!CorpNum || 0 === CorpNum.length) {
        this._throwError(-99999999, '팝빌회원 사업자번호가 입력되지 않았습니다.', err);
        return;
    }
...
BaseService.addMethod(EasyFinBankService.prototype, 'updateBankAccount', function (CorpNum, BankCode, AccountNumber, BankAccountInfo, UserID, success, error) {
    BankAccountInfo.BankCode = BankCode;
    BankAccountInfo.AccountNumber = AccountNumber;
...
BaseService.addMethod(EasyFinBankService.prototype, 'closeBankAccount', function (CorpNum, BankCode, AccountNumber, CloseType, UserID, success, error) {
    ...
    if (!CloseType || 0 === CloseType.length) {
        this._throwError(-99999999, '계좌 정액제 해지유형이 입력되지 않았습니다.', error);
        return;
    }

    if (CloseType !== "중도" && CloseType !== "일반") {
        this._throwError(-99999999, '계좌 정액제 해지유형이 올바르지 않습니다.', error);
        return;
    }
...
BaseService.addMethod(EasyFinBankService.prototype, 'revokeCloseBankAccount', function (CorpNum, BankCode, AccountNumber, UserID, success, error) {
...
BaseService.addMethod(EasyFinBankService.prototype, "getJobState", function (CorpNum, JobID, UserID, success, error) {
    if (!JobID.length) {
        this._throwError(-99999999, '작업아이디(jobID)가 입력되지 않았습니다..', error);
        return;
    }
    if (JobID.length !== 18) {
...
BaseService.addMethod(EasyFinBankService.prototype, "search", function (CorpNum, JobID, success, error) {
    this.search(CorpNum, JobID, '', '', 1, 500, 'd', "", success, error);
});
...
BaseService.addMethod(EasyFinBankService.prototype, "saveMemo", function (CorpNum, TID, Memo, UserID, success, error) {
    if (!TID) {
        this._throwError(-99999999, '거래내역 아이디가 입력되지 않았습니다..', error);
        return;
    }
    if (!Memo) {
        this._throwError(-99999999, '메모가 입력되지 않았습니다..', error);
        return;
    }
