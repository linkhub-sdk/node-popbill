...
BaseService.addMethod(TaxinvoiceService, 'getCertificateExpireDate', function(CorpNum, success, error) {
...
BaseService.addMethod(TaxinvoiceService, 'checkMgtKeyInUse', function(CorpNum, KeyType, MgtKey, success, error) {
...
BaseService.addMethod(TaxinvoiceService, 'getFiles', function(CorpNum, KeyType, MgtKey, success, error) {
...
TaxinvoiceService.prototype.search = function (CorpNum, KeyType, DType, SDate, EDate, State, Type, TaxType, LateOnly, Order, Page, PerPage, TaxRegIDType, TaxRegIDYN, TaxRegID, QString, InterOPYN, UserID, IssueType, RegType, CloseDownState, MgtKey, success, error) {
    ...
    if (!this._isNullOrEmpty(IssueType)) {
        targetURI += '&IssueType=' + IssueType.toString();
    }

    if (!this._isNullOrEmpty(RegType)) {
        targetURI += '&RegType=' + RegType.toString();
    }

    if (!this._isNullOrEmpty(CloseDownState)) {
    ...
    if (!this._isNullOrEmpty(TaxRegIDType)){
        targetURI += '&TaxRegIDType=' + TaxRegIDType;
    }

    if (!this._isNullOrEmpty(TaxRegIDYN))
        targetURI += '&TaxRegIDYN=' + TaxRegIDYN;

    if (!this._isNullOrEmpty(TaxRegID)) {
        targetURI += '&TaxRegID=' + TaxRegID;
    }

    if (!this._isNullOrEmpty(InterOPYN)) {
