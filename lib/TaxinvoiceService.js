...
BaseService.addMethod(TaxinvoiceService.prototype, 'registTaxCert', function (CorpNum, certPublicKey, certPrivateKey, certCipher, UserID, success, error) {
    if (!certPublicKey || 0 === certPublicKey.length) {
        this._throwError(-99999999, '공동인증서 공개키가 입력되지 않았습니다.', error);
        return;
    }

    if (!certPrivateKey || 0 === certPrivateKey.length) {
        this._throwError(-99999999, '공동인증서 개인키가 입력되지 않았습니다.', error);
        return;
    }

    if (!certCipher || 0 === certCipher.length) {
        this._throwError(-99999999, '공동인증서 비밀번호가 입력되지 않았습니다.', error);
        return;
    }

    if (!this._encryptPublicKeyInfo) {
        this._throwError(-99999999, 'EncryptKey 정보가 입력되지 않았습니다.', error);
        return;
    }
...
TaxinvoiceService.prototype.checkMgtKeyInUse = function (CorpNum, KeyType, MgtKey, UserID, success, error) {
    if (!KeyType || 0 === KeyType.length) {
        this._throwError(-99999999, '세금계산서유형이 입력되지 않았습니다.', error);
        return;
    }
...
BaseService.addMethod(TaxinvoiceService.prototype, "attachFile", function (CorpNum, KeyType, MgtKey, DisplayName, FilePaths, UserID, success, error) {
    ...
    if (!DisplayName || 0 === DisplayName.length) {
        this._throwError(-99999999, '첨부파일명이 입력되지 않았습니다.', error);
        return;
    }
    if (!FilePaths || 0 === FilePaths.length) {
        this._throwError(-99999999, '첨부파일 경로가 입력되지 않았습니다.', error);
        return;
    }
    var req = {
...
BaseService.addMethod(TaxinvoiceService.prototype, "getSendToNTSConfig", function (CorpNum, UserID, success, error) {
    this._executeAction({
...
BaseService.addMethod(TaxinvoiceService.prototype, "sendEmail", function (CorpNum, KeyType, MgtKey, Receiver, UserID, success, error) {
    ...
    if (!Receiver || 0 === Receiver.length) {
        this._throwError(-99999999, '수신자 이메일주소가 입력되지 않았습니다.', error);
        return;
    }
...
BaseService.addMethod(TaxinvoiceService.prototype, "sendSMS", function (CorpNum, KeyType, MgtKey, Sender, Receiver, Contents, UserID, success, error) {
    ...
    if (!Contents || 0 === Contents.length) {
        this._throwError(-99999999, '메시지 내용이 입력되지 않았습니다.', error);
        return;
    }
...
BaseService.addMethod(TaxinvoiceService.prototype, 'bulkSubmit', function (CorpNum, SubmitID, taxinvoiceList, forceIssue, UserID, success, error) {
    if (!SubmitID || 0 === SubmitID.length) {
        this._throwError(-99999999, '제출아이디가 입력되지 않았습니다.', error);
        return;
    }
    if (!taxinvoiceList || 0 === taxinvoiceList.length) {
        this._throwError(-99999999, '세금계산서 정보가 입력되지 않았습니다.', error);
        return;
    }
    var bulkRequest = {};
...
BaseService.addMethod(TaxinvoiceService.prototype, 'getBulkResult', function (CorpNum, SubmitID, UserID, success, error) {
    if (!SubmitID || 0 === SubmitID.length) {
        this._throwError(-99999999, '제출아이디가 입력되지 않았습니다.', error);
        return;
    }
