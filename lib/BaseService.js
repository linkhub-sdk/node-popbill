...
BaseService.prototype.getPartnerURL = function(CorpNum, TOGO, success, error) {
    ...
    if (this._isNullOrEmpty(TOGO)) {
        this._throwError(-99999999, 'TOGO가 입력되지 않았습니다.', error)
        return
    }
...
BaseService.addMethod(BaseService.prototype, 'getCorpInfo', function(CorpNum, UserID, success, error) {
    this.getCorpInfo(CorpNum, UserID, CorpNum, success, error)
})
...
BaseService.prototype._executeAction = function(options) {
    if (this._isNullOrEmpty(options.CorpNum)) {
        this._throwError(-99999999, '팝빌회원 사업자번호가 입력되지 않았습니다.', options.error)
        return
    }

    var boundary = '--POPBILL--NODE--'
    var isMultiPart = !!options.Files
    var isBinary = !!options.BinaryFiles
...
BaseService.prototype._throwError = function(Message, err) {
