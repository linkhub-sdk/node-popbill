...
function BaseService(config) {
    ...
    if (this._config.UseLocalTimeYN != undefined){
      this.UseLocalTimeYN = this._config.UseLocalTimeYN;;
    } else {
      this.UseLocalTimeYN = true;
    }
...
BaseService.prototype._getToken = function (CorpNum, err) {
    ...
    if (expired) {
        newToken = linkhub.newToken(this.ServiceID, CorpNum, this._getScopes(), this.IPRestrictOnOff ? null : '*');
