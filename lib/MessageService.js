...
MessageService.prototype.sendSMS = function (CorpNum, Sender, Receiver, ReceiverName, Contents, reserveDT, adsYN, SenderName, requestNum, success, error) {
    var Messages = {
        Sender: Sender,
...
MessageService.prototype.sendXMS_multi = function (CorpNum, Sender, Subject, Contents, Messages, reserveDT, adsYN, requestNum, UserID, success, error) {
...
BaseService.addMethod(MessageService.prototype, 'sendMessage', function (MsgType, CorpNum, Sender, SenderName, Subject, Contents, Messages, reserveDT, adsYN, requestNum, UserID, success, error) {
    ...
    if (this._isNullOrEmpty(MsgType)) {
        this._throwError('메시지 유형이 입력되지 않았습니다.', error);
        return;
    }

    if (this._isNullOrEmpty(Messages)) {
        this._throwError('전송할 메시지가 입력되지 않았습니다.', error);
        return;
    }

    if (!this._isNullOrEmpty(reserveDT)){
        if (!this._IsValidDT(reserveDT)){
            this._throwError('전송 예약일시 형식이 올바르지 않습니다.', error);
            return;
        }
    }
...
MessageService.prototype.sendMMS = function (CorpNum, Sender, Receiver, ReceiverName, Subject, Contents, FilePaths, reserveDT, adsYN, requestNum, UserID, success, error) {
    var Messages = {
        Sender: Sender,
        Receiver: Receiver,
        ReceiverName: ReceiverName,
...
MessageService.prototype.sendMMS_multi = function (CorpNum, Sender, Subject, Contents, Messages, FilePaths, reserveDT, adsYN, requestNum, UserID, success, error) {
...
MessageService.prototype.sendMessage_MMS = function (CorpNum, Sender, SenderName, Subject, Contents, Messages, FilePaths, reserveDT, adsYN, requestNum, UserID, success, error) {
    ...
    if (this._isNullOrEmpty(Messages)) {
        this._throwError('전송할 메시지가 입력되 않았습니다.', error);
        return;
    }
...
MessageService.prototype.search = function(CorpNum, SDate, EDate, State, Item, ReserveYN, SenderYN, Order, Page, PerPage, QString, UserID, success, error){
    ...
    if (!this._IsValidDate(SDate)){
        this._throwError('시작일자가 올바르지 않습니다.', error);
        return
    }
    ...
    if (!this._IsValidDate(EDate)){
        this._throwError('종료일자가 올바르지 않습니다.', error);
        return
    }
